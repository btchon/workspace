...
public class ClickerClientApp extends Application
{
    ...
    protected void openMouseControl(String questionNumber, String flags, String parameters, String color){
        Intent intent = new Intent(this, ClickPadActivity.class); // Changed from ClickPadActivity.class to ClickKeyboardPadActivity.class
        intent.putExtra(QUESTION_NUMBER, questionNumber);
        intent.putExtra(FLAGS, flags);
        intent.putExtra(VALUES, parameters);
        intent.putExtra(COLOR, color);
    ...
    // New added by Cody Bardell
    protected void openSpotlightControl(String questionNumber, String flags, String parameters, String color)
    {
        Intent intent = new Intent(this, SpotlightPadActivity.class);
        intent.putExtra(QUESTION_NUMBER, questionNumber);
        intent.putExtra(FLAGS, flags);
        intent.putExtra(VALUES, parameters);
        intent.putExtra(COLOR, color);
        ...
        try {
            ...
            if(!currentlyDisconnected){
                ...
                if(input.length >= 1){
                    ...
                    else if(input[0].equals(ClickerConstants.OPEN_SPOTLIGHT))
                    {
                        if(input.length<5){
                            col=DEFAULT;
                        }
                        ...
                        Log.d(ClickerConstants.TAG, "Mouse open requested with: "+str);
                        openSpotlightControl(input[1], input[2], input[3], col);
                    }
