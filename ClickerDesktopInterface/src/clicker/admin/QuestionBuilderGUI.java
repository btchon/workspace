...
//		add(questionSetPanel);
	...
	private JPanel buildNewLoadPanel()
	{
		JPanel panel = new JPanel();
		panel.setLayout(null);
		
		JButton newButton = new JButton("New");
		newButton.setBounds(panel.getLocation().x + BOX_WIDTH - (newButton.getPreferredSize().width + DIVIDER_WIDTH), panel.getLocation().y + (370/2), newButton.getPreferredSize().width, newButton.getPreferredSize().height);
		newButton.addActionListener(new NewActionListener());
		panel.add(newButton);
		
		JButton loadButton = new JButton("Load");
		loadButton.setBounds(panel.getLocation().x + BOX_WIDTH + DIVIDER_WIDTH, panel.getLocation().y + (370/2), loadButton.getPreferredSize().width, loadButton.getPreferredSize().height);
	...
	private JPanel buildLoadPanel()
	{
		...
		{
			list = new JList(lectureModel.getAllSets());
		...
		listScroller.setBounds(panel.getLocation().x + BOX_WIDTH - (listScroller.getPreferredSize().width / 2), panel.getLocation().y + (BOX_HEIGHT / 2) - (listScroller.getPreferredSize().height / 2), listScroller.getPreferredSize().width, listScroller.getPreferredSize().height);
		panel.add(listScroller);
		
		JButton loadButton = new JButton("Load");
		loadButton.setBounds(panel.getLocation().x + BOX_WIDTH - (loadButton.getPreferredSize().width / 2), listScroller.getLocation().y + listScroller.getSize().height + 10, loadButton.getPreferredSize().width, loadButton.getPreferredSize().height);
		panel.add(loadButton);
		return panel;
	}
	...
	private class LoadSetActionListener implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent e) 
		{
			hub.sendMessage(Constants.GET_ALL_QUESTIONS + Constants.GRAVE_SEPARATOR + lectureModel.getAllSets()[list.getSelectedIndex()]);
...
//		updateSetPanel();
	...
	{
		JPanel panel = new QBEditorPanel();
