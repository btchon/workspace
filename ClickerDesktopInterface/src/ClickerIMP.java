...
public class ClickerIMP extends JPanel
{
	...
	private Map<String, Class<?>> availableControls;
	...
	public ClickerIMP()
	{
		...
		availableControls = Collections.synchronizedMap(new HashMap<String, Class<?>>());
		...
		for (String s: files)
		{
			try 
			{
				...
				if (works)
				{
					...
					else if (parts.length > 1 && parts[1].equals("Control"))
					{
						availableControls.put(parts[0], consumerClass);
						System.out.println(temporaryConsumption);
					}
...
//						System.out.println("Open question: " + str);
					...
					else if (strParts[0].equalsIgnoreCase(Constants.OPEN_CLICK_PAD) 
							|| strParts[0].equalsIgnoreCase(Constants.OPEN_SPOTLIGHT_PAD))
					{
						...
						{
							String[] groupNameParts = groupName.split(Constants.COLON_SEPARATOR);
							openGroups.put(groupNameParts[0], new ConsumerGUI(groupNameParts[0], str, availableControls, strParts[2]));
						}
					}
...
//						System.out.println("Recieved: " + str);
	...
	private void closeConsumerPlugin(String groupName) 
	{
		...
		openGroups.get(groupName).close();
	}
